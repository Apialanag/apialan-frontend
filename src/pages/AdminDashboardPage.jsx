...
function AdminDashboardPage() {
  ...
  // --- Lógica de Fetching con Paginación ---
  const fetchReservas = useCallback(async (pageToFetch) => {
    ...
    try {
      ...
      setError('No se pudieron cargar las reservas. Es posible que tu sesión haya expirado.');
    } finally {
      setLoading(false);
    }
  }, [estadoFiltro, debouncedBusqueda, fechaInicioFiltro, fechaFinFiltro]);

  // Efecto para aplicar filtros (reseteando a la página 1)
  useEffect(() => {
    // Cuando un filtro cambia, queremos volver a la primera página de los nuevos resultados
    if (currentPage !== 1) {
      setCurrentPage(1);
    } else {
      fetchReservas(1);
  ...
  const getPaginationItems = () => {
    const items = [];
    if (totalPages <= 7) { // Si hay 7 páginas o menos, muéstralas todas
      for (let i = 1; i <= totalPages; i++) {
        items.push(i);
      }
    } else { // Si hay más de 7 páginas, usa la lógica de elipsis
      items.push(1); // Siempre muestra la primera página
      if (currentPage > 3) {
        items.push('...');
      }
      if (currentPage > 2) {
      ...
      items.push(totalPages); // Siempre muestra la última página
    }
  ...
  const handleCancelReserva = async (reservaId) => { 
    if (window.confirm(`¿Estás seguro de que deseas cancelar la reserva con ID ${reservaId}?`)) { 
      try { 
        await api.delete(`/reservas/${reservaId}`); 
        fetchReservas(currentPage);
      } catch (err) { 
        console.error(`Error al cancelar la reserva ${reservaId}:`, err); 
        alert(`Error al cancelar la reserva: ${err.response?.data?.error || 'Error del servidor'}`); 
      } 
    } 
  };
  
  if (loading) { return ( <div className="admin-dashboard"><h2>Panel de Administración de Reservas</h2><p>Cargando reservas...</p></div> ); }
  if (error) { return ( <div className="admin-dashboard"><h2>Panel de Administración de Reservas</h2><p className="mensaje-error">{error}</p></div> ); }
    ...
    <> 
      <div className="admin-dashboard">
        ...
        <div className="filtros-container">
          ...
          {/* Ya no necesitamos el botón "Aplicar Filtros" con el nuevo sistema de useEffect */}
